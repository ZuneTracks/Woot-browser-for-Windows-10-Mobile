...
The authoritative solution now includes a `Store | ARM` configuration. It builds
optimized, unsigned Store-upload candidates with `UapAppxPackageBuildMode=StoreUpload`
and requires `Package.StoreAssociation.xml` before packaging. Run **Publish >
Associate App with the Store** in Visual Studio after reserving the app name in
Partner Center; the repository intentionally does not invent Store identity or
publisher values. See [STORE_READINESS.md](STORE_READINESS.md) for blockers,
eligibility caveats, and the exact package command.
